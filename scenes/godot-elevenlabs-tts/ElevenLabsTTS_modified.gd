...
# Signal for when the list of voices is fetched
signal ElevenLabs_voices_fetched(names: Array, name_to_voice_id: Dictionary)

# Signal for when speech is generated
signal ElevenLabs_generated_speech

# Your Eleven Labs API key
@export var api_key = "elevenlabs_api_key"
...
func get_voices_list() -> void:
	http_request = HTTPRequest.new()
	add_child(http_request)
	http_request.connect("request_completed", Callable(self, "_on_request_completed_voices_list"))
	var error = http_request.request(endpoint_voice_list, ["xi-api-key: " + api_key,'accept: application/json'], HTTPClient.METHOD_GET)
	if error != OK:
			push_error("An error occurred in the HTTP request.")

func _on_request_completed_voices_list(result, responseCode, headers, body):
	...
	if responseCode != 200:
		...
		emit_signal("ElevenLabs_voices_fetched", [], {})
		return
	...
	# Dictionary to hold the name and voice_id
	var name_to_voice_id = {}
	# Array to hold the names
	var names = []
	# Iterate through the voices array and populate the dictionary and array
	for voice in response["voices"]:
		var name = voice["name"]
		var voice_id = voice["voice_id"]

		name_to_voice_id[name] = voice_id
		names.append(name)

	# The dictionary now contains the names and voice IDs
	print("ElevenLabs Voices Dictionary: ", name_to_voice_id)
	# The array now contains the names
	print("ElevenLabs Voices Array:", names)
	
	emit_signal("ElevenLabs_voices_fetched", names, name_to_voice_id)
	
func set_voice_id(voice: String):
	voice_id = voice
	# Endpoint and headers change depending on if using stream mode
	if use_stream_mode == true:
		endpoint = endpoint + voice_id + "/stream"
		eleven_labs_stream = AudioStreamWAV.new()
		headers = PackedStringArray(["accept: */*", "xi-api-key: " + api_key, "Content-Type: application/json"])
	else:
		endpoint = endpoint + voice_id
		eleven_labs_stream = AudioStreamMP3.new()
		headers = PackedStringArray(["accept: audio/mpeg", "xi-api-key: " + api_key, "Content-Type: application/json"])
